...
function Webot(config){
  ...
  this.routes = [];
  this.waits = {};
  this.waitBots = {}; // webots as waiters for delegation
...
/**
 *  Register a webot as wait bot or return it
 *
 *  Wait bot is used in Info.prototype.delegate to delegate all replies to another webot.
 *
 *  @param {String} name registered name, it should be unique otherwise the existing one will be overwritten
 *  @param {object/Webot} webot a webot object
 *
 *  @return {object/Webot} the wait bot
 */
Webot.prototype.waitBot = function (name, webot) {
  // adaption for waitBot object in session
  if (typeof name === 'object') {
    name = name.name;
  }

  // return registered wait bot
  if (webot === undefined) {
    return this.waitBots[name];
  }

  // register wait bot
  if (this.waitBots[name]) {
    warn('overwriting exist wait bot: %s', name);
  }
  this.waitBots[name] = webot;
...
Webot.prototype.reply = function(data, cb){
  ...
  // delegate to the active bot if exist
  if (info.session && info.session.waitBot) {
    self = this.waitBot(info.session.waitBot);
    log('replied by wait bot: ', self);
  }
  ...
  ruleList = self.befores.concat(ruleList);
