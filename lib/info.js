...
// delegate to another webot
// if not specified, abort delegation and revert to parent
// @param name name of the target webot, registered by Webot.waitBot
Info.prototype.delegate = function (name) {
  var oldBot = this.session.waitBot;
  if (name) {
    this.session.waitBot = {name: name};
    if (oldBot) {
      this.session.waitBot.parent = oldBot;
    }
  } else {
    this.session.waitBot = oldBot && oldBot.parent || undefined;
  }
