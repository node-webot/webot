...
function Rule(cfg, parent){
  if (cfg instanceof Rule) return cfg;
  if (!(this instanceof Rule)) return new Rule(cfg, parent);
...
Rule.convert = function(cfg){
  ...
  switch(typeof cfg) {
    ...
    case 'object':
      ...
      if ('handler' in cfg) return [new Rule(cfg)];
      break;
    default:
      return []
  }
...
Rule.prototype.test = function(info){
  ...
  if (!p && p !== false) return true;

  // call pattern is it's a function
  if(typeof p === 'function') return p.call(rule, info);
  ...
  verbose('[%s] has an unsupported pattern.', rule.name);
  return false;
};
